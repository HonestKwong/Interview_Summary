/***********************************************************
  ...
  > Created Time: 2021年02月20日 星期六 13时16分21秒
    > Modified Time:2021年02月20日 星期六 17时03分39秒
 *******************************************************/
...
void http_request(int cfd, const char* file)
{
    ...
    if(S_ISREG(sbuf.st_mode)) {//是一个普通文件
        ...
        //send_respond(cfd, 200, "OK", "Content-Type: text/plain; charset=iso-8869-1", sbuf.st_size);
        send_respond(cfd, 200, "OK", "Content-Type: audio/mpeg; charset=iso-8869-1", sbuf.st_size);
        //回发客户端请求数据内容
...
void do_read(int cfd, int epfd)
{
    ...
    if (len == 0) {
        ...
        printf("method = %s, path = %s , protocal = %s", method, path, protocol);
