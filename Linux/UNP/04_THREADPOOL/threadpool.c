/***********************************************************
  ...
  > Created Time: 2021年02月16日 星期二 10时57分26秒
    > Modified Time:2021年02月17日 星期三 11时14分06秒
 *******************************************************/
...
threadpool_t* threadpool_create(int min_thr_num, int max_thr_num, int queue_max_size)
{
    ...
    do{
        ...
        pool->busy_thr_num = 0;
        pool->live_thr_num = min_thr_num;      //或者的线程数 初值 = 最小线程数
        ...
        return pool;
    } while (0);         //使用while(0) 可以保证前面错误时可以通过break跳过后续步骤直接释放线程池
...
//向线程池中 添加一个任务
//threadpool_add(thp, process, (void*)&num[i]);    //向线程池中添加任务 procss: 小写--->大写
...
void* threadpool_thread(void* threadpool)
{
    ...
    while(true) {
        ...
        // 如果 指定了true 要关闭线程池里的每个线程, 自行退出处理--销毁线程池
        if (pool->shutdown) {
            pthread_mutex_unlock(&(pool->lock));
            printf("thread 0x%x is exiting\n", (unsigned int)pthread_self());
...
void* adjust_thread(void* threadpool)
{
    ...
    while(!pool->shutdown) {
        ...
        //销毁多余的空闲线程 算法: 忙线程 * 2 小于 存活的线程数 且 存活的线程数 大于最小线程数时
        if ((busy_thr_num * 2) < live_thr_num && live_thr_num > pool->min_thr_num) {

            // 一次销毁 DEFAULT_THREAD个线程, 随机10个即可
            pthread_mutex_lock(&(pool->lock));
